...
public class Drooler {
	...
	private StatelessKnowledgeSession projectJobSchedulingKsession;

	public Drooler(String[] initRules) {
		try {
			
			//loads default drools file
			this.ruleSource = initRules;
			this.kbuilder = KnowledgeBuilderFactory.newKnowledgeBuilder();
			for (int i = 0; i < initRules.length; i++) {
				
				System.out.println("Rule File Loaded = " + initRules[i]);
			...
			this.projectJobSchedulingKsession = this.kbase.newStatelessKnowledgeSession();
		} catch (Exception e) {
	...
	public StatelessKnowledgeSession getKsession() {
		return projectJobSchedulingKsession;
	}

	public void setKsession(StatelessKnowledgeSession projectJobSchedulingKsession) {
		this.projectJobSchedulingKsession = projectJobSchedulingKsession;
	}
