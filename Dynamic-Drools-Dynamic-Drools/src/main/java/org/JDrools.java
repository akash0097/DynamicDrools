...
public class JDrools extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.startEngine) {
			...
			try {
				...
				this.drools = new Drooler(new String[] { JDrools.class.getResource("JDrools.class").getPath()
						.replace("/target/classes/org/JDrools.class", "/src/main/java/org/basic.drl") });
		...
		if (e.getSource() == this.addRule) {
			...
			System.out.println("Inside Adding rule Action");
			...
			if (option == JFileChooser.APPROVE_OPTION) {
				try {
					...
					if (kbuild.hasErrors()) {
						...
						//adding new rules
						System.out.println("session " + drools.getKsession());
	...
	public void refreshList() {
		...
		System.out.println("Refresh the list in drop down");
